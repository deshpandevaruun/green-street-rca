...
def buyer_seller_match(merged):
    ...
    Returns dataframe with similar buyers and sellers
    -------
    None.
    ...
    # Apply the sematics_score for each pair across all rows
    for col1, col2 in column_pairs_buyers:
        comparison_results_buyers[f'{col1}_vs_{col2}'] = merged.apply(lambda row: semantics_score(row[col1], row[col2]), axis=1)
        
    for col1, col2 in column_pairs_sellers:
        comparison_results_sellers[f'{col1}_vs_{col2}'] = merged.apply(lambda row: semantics_score(row[col1], row[col2]), axis=1) 
    ...
    return max_comparison_buyers, max_comparison_sellers
...
def main():
    ...
    buyer_seller_match(step_2_merged_30_days)
